...
class JobTest < MiniTest::Test
  def test_wage_growth
    ...
    assert_equal(
      7_000_00,
      today.calculate_take_home_pay(Date.today, growth_rates: growth_rates)
    )

    next_year = job_plan.to_reader.on(Date.today + 1.year)
    assert_equal(
      7_140_00,
      next_year.calculate_take_home_pay(Date.today + 1.year, growth_rates: growth_rates)
    )

    next_year = job_plan.to_reader.on(Date.today + 2.years)
    assert_equal(
      7_354_20,
      next_year.calculate_take_home_pay(Date.today + 2.years, growth_rates: growth_rates)
    )
  end
